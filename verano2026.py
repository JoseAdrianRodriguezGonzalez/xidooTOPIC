...
def find_optimal_resolution( graph, emb, res_min=0.4, res_max=1.4, step=0.05, alpha=0.25, min_avg_size=5):
    """
    Búsqueda en rejilla para encontrar resolución óptima en Leiden.
    Optimiza coherencia + separación - penalización por fragmentación.
    """
    ...
    print("Explorando resoluciones...")

    for res in resolutions:
        if res <= 0:
            continue  # seguridad extra

        labels, n_clusters = run_leiden(graph, res)
        ...
        # Evitar micro-fragmentación extrema
        if avg_cluster_size < min_avg_size:
            continue

        coh = compute_intra_coherence(emb, labels)
        sep = compute_inter_separation(emb, labels)

        cluster_penalty = alpha * (n_clusters / np.sqrt(N))
    ...
    if best_res is None:
        print("Advertencia: No se encontró una resolución óptima válida dentro del rango especificado y los criterios.")
        print("Considera ajustar 'res_min', 'res_max', 'step', 'alpha' o 'min_avg_size'.")
        return None, None # Return None for labels and optimal_res if not found

    print("\nResolución óptima encontrada:")
    print(f"γ = {best_res:.3f}")
    print(f"Clusters obtenidos = {len(np.unique(best_labels))}")
