...
Original file is located at
    https://colab.research.google.com/drive/1bBRMkUhpi4v3PcEfRqzm0URCMQfps86B

#Instala todo
"""

!pip install transformers accelerate faiss-cpu leidenalg igraph keybert spacy
...
def estimate_optimal_neighbors(
    ...
    """
    Estima número óptimo de vecinos para grafo k-NN
    basado en conectividad y estabilidad estructural.
    """
...
def remove_outliers_isolation_forest(embeddings: np.ndarray, texts: list, contamination: float = 0.02, random_state: int = 42):
    """
    Detecta y elimina anomalías en embeddings usando Isolation Forest.

    Parámetros:
    -----------
    embeddings : np.ndarray
        Matriz (n_samples, embedding_dim)
    texts : list[str]
        Lista de textos correspondientes a los embeddings
    contamination : float
    ...
    stats = {
        ...
        "removed_percentage": round(len(removed_indices) / len(texts) * 100, 2)
    }

    return filtered_embeddings, filtered_texts, removed_indices, scores, stats
...
def leiden_clustering(graph, resolution: float = 1.0, random_state: int = 42):
    """
    Aplica algoritmo Leiden para detectar comunidades.
    """
